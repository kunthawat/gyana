...
class FusionCharts:
    ...
    def render(self):
        ...
        for key, value in self.eventOptions.items():
            ...
            self.readyJson = self.readyJson.replace("_fceventname_", key).replace(
                "_fceventbody_", value
            )
...
class FusionTable:
    ...
    def Select(self, *columnName):
        if len(columnName) > 0:
            ...
            self.stringBuilder.AppendLine(
                "fusionTable = fusionTable.query(FusionCharts.DataStore.Operators.select(["
                + columns
                + "]));"
            )
    ...
    def CreateFilter(self, filterType, columnName, *values):
        ...
        return switcher.get(
            filterType,
            "FusionCharts.DataStore.Operators.{0}('{1}', {2})".format(
                fx, columnName, my_list[0]
            ),
        )
    ...
    def Pipe(self, *filters):
        if len(filters) > 0:
            ...
            self.stringBuilder.AppendLine(
                "fusionTable = fusionTable.query(FusionCharts.DataStore.Operators.pipe("
                + filter
                + "));"
            )

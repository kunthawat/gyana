...
describe('nodes', () => {
  ...
  it('input', () => {
    ...
    cy.contains('revenue').click({ turbo: false })
    cy.contains('Loading preview...').should('be.visible')
    cy.get('div[class=placeholder-scr__title]').should('not.exist')
    cy.contains('Save & Preview').should('not.be.disabled').click({ turbo: false })
  ...
  it('select', () => {
    ...
    cy.contains('Save & Preview').should('be.disabled')
    cy.contains('Save & Preview').click()
